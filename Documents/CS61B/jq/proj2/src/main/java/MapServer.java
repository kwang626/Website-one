...
public class MapServer {
    ...
    public static BufferedImage getMapRaster(Map<String, Double> inputParams,
                                             Map<String, Object> rasteredImageParams) throws IOException {
        QuadTree x = new QuadTree();
        double target = findtargetratio(inputParams);
        ArrayList<QTreeNode> list = x.root.getAllTiles(inputParams, target);
        list.sort(blackMagic);
        double ullat = inputParams.get("ullat");
        int columns = 0;
        for (int a = 0; a < list.size(); a++) {
            if (list.get(a).coords.get("ullat") > ullat) {
                columns++;
            }
        }
        double raster_ullon = list.get(0).coords.get("ullon");
        double raster_ullat = list.get(0).coords.get("ullat");
        double raster_lrlon = list.get(list.size() - 1).coords.get("lrlon");
        double raster_lrlat = list.get(list.size() - 1).coords.get("lrlat");
        rasteredImageParams.put("raster_ul_lon", raster_ullon);
        rasteredImageParams.put("raster_ul_lat", raster_ullat);
        rasteredImageParams.put("raster_lr_lon", raster_lrlon);
        rasteredImageParams.put("raster_lr_lat", raster_lrlat);
        int rows = list.size() / columns;
        int depth = list.get(0).name.length();
        rasteredImageParams.put("depth", depth);
        rasteredImageParams.put("query_success", true);
        File[] pictures = new File[rows * columns];
        for (int z = 0; z < rows * columns; z++) {
            pictures[z] = new File("img/" + list.get(z).name + ".png");
        }
        BufferedImage[] pic = new BufferedImage[rows * columns];
        for (int z = 0; z < rows * columns; z++) {
            try{
                pic[z] = ImageIO.read(pictures[z]);
            } catch (IOException e){
                return null;
            }
        }
        int picWidth = pic[0].getWidth() * columns;
        int picHeight = pic[0].getHeight() * rows;
        rasteredImageParams.put("raster_width", picWidth);
        rasteredImageParams.put("raster_height", picHeight);
        BufferedImage result = new BufferedImage(picWidth, picHeight,
                pic[0].getType());
        int number = 0;
        for (int q = 0; q < rows; q++) {
            for (int w = 0; w < columns; w++) {
                result.createGraphics().drawImage(pic[number], pic[0].getWidth() * w,
                        pic[0].getHeight() * q, null);
                number++;
            }
        }
...
//       input = {lrlon=-122.2119140625, w=751.0, ullon=-122.24274008330374, h=517.0, ullat=37.85175717488349, lrlat=37.826037629222625}.
//
//    Expected: {raster_ul_lon=-122.244873046875, raster_height=1792, depth=4, raster_lr_lon=-122.2119140625,
//            raster_lr_lat=37.82280243352756, raster_ul_lat=37.85316192077866, query_success=true, raster_width=1536}
//
//    but got: {raster_ul_lon=-122.244873046875, raster_height=1792, depth=4, raster_lr_lon=-122.2393798828125,
//            raster_lr_lat=37.848824851171365, raster_ul_lat=37.85316192077866, raster_width=1536}.
//


    //lrlon=-122.2119140625, w=751.0, ullon=-122.24274008330374, h=517.0, ullat=37.85175717488349, lrlat=37.826037629222625
    ...
    public static Comparator<QTreeNode> blackMagic = new Comparator<QTreeNode>() {
        @Override
    ...
    public static double findtargetratio(Map<String, Double> input) {
        double temp = input.get("lrlon") - input.get("ullon");
        temp = temp / input.get("w");
        return temp;
    }
